...
颈主要是存在于系统的哪个方面。
    ...
    def summarize_bottlenecks(
        ...
        bottle_neck_prompt = f"""
        # CONTEXT #
        当前 Linux 系统的性能分析报告如下，报告中所涉及的数据准确无误，真实可信：
        {report}

        # OBJECTIVE #
        请根据系统性能分析报告，对系统可能存在的性能瓶颈进行量化评分。你需要对以下五个方面分别给出评分（0~1），表示当前系统在该方面的性能压力或瓶颈程度：
        [CPU, NETWORK, DISK, MEMORY, NONE]

        评分要求：
        1. 0 表示没有瓶颈，1 表示严重瓶颈。
        2. 你必须从多个指标和多条信息进行综合判断，不要基于单点信息轻易下结论。
        3. NONE 的评分表示整体系统没有明显瓶颈，如果系统存在瓶颈，NONE 应为 0。
