...
def get_messages(role_prompt: str, history: str, usr_prompt: str) -> list:
    """
...
def build_index(file_name):
    ...
    for doc in tqdm(docs, desc=f"Building index for {file_name}..."):
        query_embedding = generate_embeddings(doc["info"]["desc"])
...
def retrieve_faiss(index, docs, query_list, topk=15, threshold=0.7):
    """
    ...
    for query_data in query_list:
        query_embedding = generate_embeddings(query_data)
...
def retrieve_bm25(bm25, tokenized_corpus, docs, query_list, topk=15):
    """
...
def generate_answer(query, static_profile, retrieved_parameters_faiss, retrieved_parameters_bm25):
    """
    ...
    for param in retrieved_parameters_faiss + retrieved_parameters_bm25:
        if param["name"] not in unique_names:
            related_names = get_related_parameter_names(param["name"])
...
def generate_bm25_keywords(performance_report_text: str, max_keywords=5):
    """
...
def split_performance_report_to_queries(report_text: str) -> list:
    """
